                ...
                <div class="overflow-x-auto">
                    <table class="w-full border-collapse text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-150">
                            ...
                            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr x-show="shouldShowCategory('<?php echo e($category->slug); ?>')" class="bg-slate-50 font-bold text-slate-700 border-b border-slate-200/80 select-none">
                            ...
                            <tr x-show="shouldShowProduct('<?php echo e($category->slug); ?>', '<?php echo e(addslashes($product->name)); ?>')" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-3.5 px-4 flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest"><?php echo e($category->name); ?></span>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-4 text-center">
                                    <div class="inline-flex items-center bg-slate-100 border border-slate-200 rounded-lg p-1 select-none">
                                        <button @click="decreaseQty(<?php echo e($product->id); ?>)" class="w-7 h-7 text-slate-650 hover:text-slate-900 hover:bg-white rounded flex items-center justify-center font-bold text-xs transition-colors shadow-sm">
                                            <i class="fa-solid fa-minus text-[9px]"></i>
                                        </button>
                                        <input @input="updateInputQty(<?php echo e($product->id); ?>, $event.target.value)" type="number" :value="cart[<?php echo e($product->id); ?>]?.qty || 0" min="0" class="w-12 text-center bg-transparent border-0 text-xs font-black text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
                                        <button @click="increaseQty(<?php echo e($product->id); ?>, <?php echo e($product->mrp); ?>, <?php echo e($product->selling_price); ?>, '<?php echo e(addslashes($product->name)); ?>', '<?php echo e($product->pack_size); ?>')" class="w-7 h-7 text-slate-650 hover:text-slate-900 hover:bg-white rounded flex items-center justify-center font-bold text-xs transition-colors shadow-sm">
    ...
    <div x-show="totalQty > 0" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-y-full opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-full opacity-0" class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200/80 shadow-2xl py-4 backdrop-blur-md px-4 select-none">
        <div class="container mx-auto max-w-5xl flex flex-col md:flex-row gap-4 items-center justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto items-center">
                ...
                <div class="w-full sm:w-48 text-center space-y-1.5" x-show="totalNet < <?php echo e($settings['min_order_value']); ?>">
