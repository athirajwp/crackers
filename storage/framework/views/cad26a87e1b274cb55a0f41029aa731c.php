...
<?php $__env->startSection('title', App\Models\Setting::get('store_name', 'Cracker Demo') . ' | Sivakasi Online Fireworks Booking Shop'); ?>
...
<div x-data="storefrontData()" x-init="initStorefront()" class="relative text-slate-800">
    ...
    <div class="lg:hidden sticky top-[64px] z-30 bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 py-2.5 shadow-sm select-none overflow-x-auto scrollbar-none flex gap-1.5 items-center">
        ...
        <button @click="activeCategory = '<?php echo e($category->slug); ?>'" :class="activeCategory === '<?php echo e($category->slug); ?>' ? 'bg-crimson-600 text-white font-extrabold shadow' : 'bg-slate-50 text-slate-650 border border-slate-200'" class="px-3.5 py-1.5 rounded-full text-[10px] uppercase tracking-wider whitespace-nowrap transition-all duration-200">
        ...
        <aside class="hidden lg:block lg:w-64 flex-shrink-0 lg:sticky lg:top-24 space-y-4 select-none">
            <div class="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm">
                ...
                <div class="flex flex-row lg:flex-col overflow-x-auto lg:overflow-visible gap-1 pb-2 lg:pb-0 scrollbar-none">
                    ...
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button @click="activeCategory = '<?php echo e($category->slug); ?>'" :class="activeCategory === '<?php echo e($category->slug); ?>' ? 'bg-crimson-600 text-white font-extrabold shadow' : 'text-slate-650 hover:bg-slate-100'" class="w-auto lg:w-full text-left px-3.5 py-2.5 rounded-xl text-xs flex items-center gap-2 whitespace-nowrap transition-all duration-200">
                        <i class="fa-solid fa-fire-flame-curved text-[11px] opacity-80"></i> <?php echo e($category->name); ?>
            ...
            <div class="hidden lg:block bg-white border border-slate-200 p-4 rounded-2xl space-y-3.5 shadow-sm">
                ...
                <div class="text-[11px] text-slate-500 space-y-2.5 leading-relaxed font-medium">
                    <div class="flex gap-2">
                        ...
                        <span><strong>Address:</strong> <?php echo e($settings['store_address']); ?></span>
                    </div>
                    <div class="flex gap-2">
                        <i class="fa-solid fa-envelope text-crimson-500/80 mt-0.5"></i>
                        <span><strong>Support:</strong> <?php echo e($settings['store_email']); ?></span>
                    </div>
                </div>
            </div>
        </aside>
                ...
                <div class="overflow-x-auto sm:overflow-visible">
                    <table class="w-full border-collapse text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-150">
                            ...
                            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr x-show="shouldShowCategory('<?php echo e($category->slug); ?>')" @click="toggleCategory('<?php echo e($category->slug); ?>')" class="bg-slate-50 font-bold text-slate-700 border-b border-slate-200/80 select-none cursor-pointer hover:bg-slate-100 transition-colors">
                                <td colspan="6" class="py-3 px-3 sm:px-4 flex items-center justify-between text-crimson-650 tracking-wider">
                                    ...
                                    <i class="fa-solid fa-chevron-down text-[10px] text-slate-400 transition-transform duration-200" :class="collapsedCategories.has('<?php echo e($category->slug); ?>') ? '-rotate-90' : 'rotate-0'"></i>
                                </td>
                            </tr>

                            <?php $__currentLoopData = $category->products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr x-show="shouldShowProduct('<?php echo e($category->slug); ?>', '<?php echo e(addslashes($product->name)); ?>')" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="py-3.5 px-3 sm:px-4 flex items-center gap-2 sm:gap-3">
                                    ...
                                    <div class="space-y-1">
                                        <h4 class="font-extrabold text-slate-800 text-xs leading-normal"><?php echo e($product->name); ?></h4>
                                        <div class="flex flex-wrap items-center gap-1.5">
                                            <span class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest"><?php echo e($category->name); ?></span>
                                            <!-- Responsive Inline Pack size badge on mobile -->
                                            <span class="sm:hidden text-[9px] font-bold text-slate-500 bg-slate-100 border border-slate-150 px-1.5 py-0.5 rounded-md font-mono"><?php echo e($product->pack_size); ?></span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="py-3.5 px-3 sm:px-4 text-right">
                                    <div class="text-slate-400 text-[10px] line-through">₹<?php echo e(number_format($product->mrp, 2)); ?></div>
                                    <div class="text-crimson-650 font-extrabold">₹<?php echo e(number_format($product->selling_price, 2)); ?></div>
                                </td>
                                ...
                                <td class="py-3.5 px-3 sm:px-4 text-center">
                                    <div class="inline-flex items-center bg-slate-100 border border-slate-200 rounded-lg p-0.5 sm:p-1 select-none">
                                        <button @click="decreaseQty(<?php echo e($product->id); ?>)" class="w-6 h-6 sm:w-7 sm:h-7 text-slate-650 hover:text-slate-900 hover:bg-white rounded flex items-center justify-center font-bold text-xs transition-colors shadow-sm">
                                            <i class="fa-solid fa-minus text-[8px] sm:text-[9px]"></i>
                                        </button>
                                        <input @input="updateInputQty(<?php echo e($product->id); ?>, $event.target.value, <?php echo e($product->mrp); ?>, <?php echo e($product->selling_price); ?>, '<?php echo e(addslashes($product->name)); ?>', '<?php echo e($product->pack_size); ?>')" type="number" :value="cart[<?php echo e($product->id); ?>]?.qty || 0" min="0" class="w-8 sm:w-12 text-center bg-transparent border-0 text-xs font-black text-slate-800 placeholder-slate-400 focus:ring-0 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none">
                                        <button @click="increaseQty(<?php echo e($product->id); ?>, <?php echo e($product->mrp); ?>, <?php echo e($product->selling_price); ?>, '<?php echo e(addslashes($product->name)); ?>', '<?php echo e($product->pack_size); ?>')" class="w-6 h-6 sm:w-7 sm:h-7 text-slate-650 hover:text-slate-900 hover:bg-white rounded flex items-center justify-center font-bold text-xs transition-colors shadow-sm">
                                ...
                                <td class="hidden md:table-cell py-3.5 px-4 text-right font-extrabold text-slate-800 pr-6">
                                    ₹<span x-text="formatCurrency((cart[<?php echo e($product->id); ?>]?.qty || 0) * <?php echo e($product->selling_price); ?>)">0.00</span>
                                </td>

                                <!-- Sub Total (always visible) -->
                                <td class="py-3.5 px-3 sm:px-4 text-right pr-4">
                                    <span class="font-extrabold text-crimson-600" x-text="(cart[<?php echo e($product->id); ?>]?.qty || 0) > 0 ? '₹' + formatCurrency((cart[<?php echo e($product->id); ?>]?.qty || 0) * <?php echo e($product->selling_price); ?>) : '—'">—</span>
                                </td>
    ...
    <div x-show="totalQty > 0" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-y-full opacity-0" x-transition:enter-end="translate-y-0 opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-y-0 opacity-100" x-transition:leave-end="translate-y-full opacity-0" class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200/80 shadow-2xl py-2.5 backdrop-blur-md px-4 select-none">
        <div class="container mx-auto max-w-5xl flex flex-col lg:flex-row gap-3 items-center justify-between">
            ...
            <div class="flex flex-col sm:flex-row gap-2.5 w-full lg:w-auto items-center justify-center lg:justify-end">
                ...
                <div class="w-full sm:w-44 text-center space-y-1" x-show="enableMinOrder && totalNet < <?php echo e($settings['min_order_value']); ?>">
...
<script>
    function storefrontData() {
        return {
            ...
            enableMinOrder: '<?php echo e($settings['enable_min_order'] ?? 'yes'); ?>' === 'yes',
            enablePromoCodes: '<?php echo e($settings['enable_promo_codes'] ?? 'yes'); ?>' === 'yes',
            enableTaxDelivery: '<?php echo e($settings['enable_tax_delivery'] ?? 'no'); ?>' === 'yes',
            taxPercent: parseFloat('<?php echo e($settings['tax_percent'] ?? 18); ?>'),
            deliveryCharge: parseFloat('<?php echo e($settings['delivery_charge'] ?? 150); ?>'),
            
            promoCodes: [
                { code: '<?php echo e($settings['promo_code_1'] ?? ''); ?>', value: '<?php echo e($settings['promo_value_1'] ?? ''); ?>' },
                { code: '<?php echo e($settings['promo_code_2'] ?? ''); ?>', value: '<?php echo e($settings['promo_value_2'] ?? ''); ?>' },
                { code: '<?php echo e($settings['promo_code_3'] ?? ''); ?>', value: '<?php echo e($settings['promo_value_3'] ?? ''); ?>' },
                { code: '<?php echo e($settings['promo_code_4'] ?? ''); ?>', value: '<?php echo e($settings['promo_value_4'] ?? ''); ?>' },
                { code: '<?php echo e($settings['promo_code_5'] ?? ''); ?>', value: '<?php echo e($settings['promo_value_5'] ?? ''); ?>' },
            ],
            ...
            minOrderProgressPercent() {
                const min = <?php echo e($settings['min_order_value']); ?>;
            ...
            openCheckoutDrawer() {
                if (!this.enableMinOrder || this.totalNet >= <?php echo e($settings['min_order_value']); ?>) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Athi\OneDrive\Desktop\crackers\resources\views/storefront.blade.php ENDPATH**/ ?>