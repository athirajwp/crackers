...
<?php $__env->startSection('title', 'Website Overview | Super Admin Console'); ?>
...
<div x-data="companyManagement()" class="space-y-6 select-none">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 select-none">
        ...
        <button @click="openAddModal()" class="bg-<?php echo e($currentTheme['accent']); ?> hover:bg-<?php echo e($currentTheme['accent_hover']); ?> text-<?php echo e($theme === 'gold' ? 'slate-950' : 'white'); ?> font-extrabold text-xs uppercase tracking-wider px-4 py-2.5 rounded-xl transition-all shadow shadow-<?php echo e($currentTheme['accent']); ?>/20 flex items-center gap-1.5 active:scale-95">
    ...
    <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6">
        ...
        <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
            <div class="flex items-center gap-2 text-xs font-semibold text-slate-500">
                <span>Show</span>
                <select x-model="entriesLimit" class="bg-slate-50 border border-slate-200 rounded-xl px-2.5 py-1.5 focus:outline-none focus:border-<?php echo e($currentTheme['accent']); ?> transition-colors text-slate-850 font-bold">
            ...
            <div class="relative w-full md:max-w-md">
                ...
                <input x-model="searchQuery" type="text" placeholder="Search by Name, Status etc..." class="w-full bg-slate-50 border border-slate-200 focus:border-<?php echo e($currentTheme['accent']); ?> focus:bg-white rounded-xl py-2.5 pl-10 pr-4 text-xs text-slate-750 placeholder-slate-400 focus:outline-none transition-all font-semibold shadow-inner">
            </div>
        </div>
        ...
        <div class="border border-slate-150 rounded-2xl overflow-hidden shadow-inner">
            <div class="overflow-x-auto">
                <table class="w-full border-collapse text-left text-xs">
                    <thead>
                        <tr class="<?php echo e($currentTheme['thead']); ?> font-black uppercase tracking-wider text-[10px] select-none">
                    ...
                    <tbody class="divide-y divide-slate-200 font-semibold">
                        ...
                        <template x-for="(company, index) in filteredCompanies" :key="company.id">
                            <tr :class="index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'" class="hover:bg-blue-50/20 transition-colors">
                                ...
                                <!-- Website Domain -->
                                <td class="py-3.5 px-4 border-r border-slate-150 font-mono text-<?php echo e($currentTheme['accent']); ?> hover:underline">
    ...
    <div x-show="modalOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="modalOpen" x-transition class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white border border-slate-200 rounded-3xl shadow-2xl sm:my-8 sm:align-middle sm:max-w-5xl sm:w-full">
                
                <!-- Modal Header -->
                <div class="<?php echo e($currentTheme['thead']); ?> px-6 py-4 flex items-center justify-between">
                ...
                <form :action="isEditMode ? '/admin_sys/company/' + form.id + '/update' : '/admin_sys/company'" method="POST" enctype="multipart/form-data" class="p-6 space-y-7 text-xs font-semibold max-h-[80vh] overflow-y-auto">
                    ...
                    <div class="border border-slate-200 rounded-3xl p-5 space-y-6 relative">
                        ...
                        <div class="border border-slate-150 rounded-2xl p-4 bg-slate-50/50 space-y-4">
                            <span class="text-[9px] uppercase tracking-wider font-extrabold text-slate-400 block border-b border-slate-200 pb-1.5 flex justify-between items-center">
                                <span>Bank Details <?php echo e($b); ?></span>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 items-center">
                                ...
                                <div class="col-span-1 flex items-center gap-3">
                                    <div class="w-16 h-16 rounded-xl bg-white border border-slate-200 overflow-hidden flex items-center justify-center text-slate-300 flex-shrink-0 shadow-inner">
                                        <template x-if="form.bank_qr_<?php echo e($b); ?>">
                                            <img :src="'/' + form.bank_qr_<?php echo e($b); ?>" class="object-cover w-full h-full">
                                        </template>
                                        <template x-if="!form.bank_qr_<?php echo e($b); ?>">
                                    ...
                                    <div class="relative w-full">
                                        <label class="absolute -top-2 left-3 bg-white px-1 text-[8px] text-slate-400 font-bold uppercase tracking-wider">Add QR Code <?php echo e($b); ?></label>
                                        <input type="file" name="bank_qr_<?php echo e($b); ?>" class="w-full bg-white border border-slate-200 rounded-lg px-2 py-1 text-[9px] text-slate-500 focus:outline-none">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">Bank Name <?php echo e($b); ?></label>
                                    <input type="text" name="bank_name_<?php echo e($b); ?>" :value="form.bank_name_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none">
                                </div>
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">IFSC Code <?php echo e($b); ?></label>
                                    <input type="text" name="bank_ifsc_<?php echo e($b); ?>" :value="form.bank_ifsc_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none font-mono uppercase">
                                </div>
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">Account No. <?php echo e($b); ?></label>
                                    <input type="text" name="bank_acc_<?php echo e($b); ?>" :value="form.bank_acc_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none font-mono">
                                </div>
                            </div>

                            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">Branch <?php echo e($b); ?></label>
                                    <input type="text" name="bank_branch_<?php echo e($b); ?>" :value="form.bank_branch_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none">
                                </div>
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">Account Type <?php echo e($b); ?></label>
                                    <input type="text" name="bank_type_<?php echo e($b); ?>" :value="form.bank_type_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none">
                                </div>
                                <div class="relative">
                                    <label class="absolute -top-2 left-3 bg-white px-1.5 text-[9px] text-slate-500 font-bold uppercase tracking-wider">Holder Name <?php echo e($b); ?></label>
                                    <input type="text" name="bank_holder_<?php echo e($b); ?>" :value="form.bank_holder_<?php echo e($b); ?>" class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none">
                                </div>
                            </div>
                        </div>
                        <?php endfor; ?>
                    ...
                    <div class="flex justify-end gap-3 pt-5 border-t border-slate-150 select-none">
                        ...
                        <button type="submit" class="bg-<?php echo e($currentTheme['accent']); ?> hover:bg-<?php echo e($currentTheme['accent_hover']); ?> text-<?php echo e($theme === 'gold' ? 'slate-950' : 'white'); ?> font-extrabold px-6 py-2.5 rounded-xl transition-all shadow shadow-<?php echo e($currentTheme['accent']); ?>/10 select-none" x-text="isEditMode ? 'Save changes' : 'Add Company'"></button>
                    </div>
...
<?php echo $__env->make('layouts.admin_sys', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Athi\OneDrive\Desktop\crackers\resources\views/admin_sys/company.blade.php ENDPATH**/ ?>