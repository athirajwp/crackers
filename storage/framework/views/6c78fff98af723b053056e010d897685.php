<?php $__env->startSection('title', 'Manage Categories | Admin Console'); ?>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
        ...
        <div class="mb-5 flex flex-col sm:flex-row gap-4 items-center justify-between">
            ...
            <div class="text-[10px] text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1.5 bg-slate-50 border border-slate-150 px-3 py-1.5 rounded-lg shadow-sm">
                ...
                <span x-text="searchQuery.trim() === '' ? <?php echo e($categories->count()); ?> : Object.values(matches).filter(Boolean).length" class="text-slate-700 font-extrabold font-mono"></span> of <span class="text-slate-700 font-extrabold font-mono"><?php echo e($categories->count()); ?></span> categories matching
            </div>
        </div>
            ...
            <table class="w-full text-left text-xs border-collapse">
                ...
                <tbody class="divide-y divide-slate-150 text-slate-650 font-semibold">
                    <?php if($categories->count() > 0): ?>
                        ...
                        <tr x-show="isMatch(<?php echo e($category->id); ?>, '<?php echo e(addslashes($category->name)); ?>', '<?php echo e(addslashes($category->slug)); ?>')" class="hover:bg-slate-50/50">
                            
                            <!-- Index -->
                            <td class="py-3.5 px-4 font-mono font-medium text-slate-400"><?php echo e($sno++); ?></td>
                            
                            <!-- Name -->
                            <td class="py-3.5 px-4">
                                <strong class="text-slate-800 font-bold text-xs select-all"><?php echo e($category->name); ?></strong>
                                <div class="text-[9px] text-slate-450 font-mono select-all"><?php echo e($category->slug); ?></div>
                            </td>
                            ...
                            <td class="py-3.5 px-4 text-center">
                                <span class="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider 
                                      <?php echo e($category->status === 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-slate-100 text-slate-450 border border-slate-200'); ?>">
                            ...
                            <td class="py-3.5 px-4 text-center pr-4">
                                <div class="inline-flex gap-2">
                                    <button @click="editItem = { 
                                                ...
                                                sort_order: <?php echo e($category->sort_order); ?>, 
                                                status: '<?php echo e($category->status); ?>' 
                                    ...
                                    <form action="<?php echo e(route('admin.categories.destroy', $category->id)); ?>" method="POST" onsubmit="return confirm('Deleting a category will delete all products inside it! Are you sure?');">
                                        <?php echo csrf_field(); ?>
        ...
        <div class="absolute inset-y-0 right-0 max-w-full flex pl-10">
            <div class="w-screen max-w-md">
                <div class="h-full flex flex-col bg-white border-l border-slate-200 shadow-2xl overflow-y-auto">
                    ...
                    <form action="<?php echo e(route('admin.categories.store')); ?>" method="POST" class="p-6 space-y-4 text-xs font-semibold">
                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Athi\OneDrive\Desktop\crackers\resources\views/admin/categories/index.blade.php ENDPATH**/ ?>