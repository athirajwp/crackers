<?php $__env->startSection('title', 'Edit Order Items | Admin Console'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 select-none">
        <div>
            <h2 class="text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
                <a href="<?php echo e(route('admin.orders.show', $order->id)); ?>" class="text-slate-400 hover:text-slate-650">
                    <i class="fa-solid fa-arrow-left"></i>
                </a>
                <span>Edit Order Items</span>
            </h2>
            <p class="text-[10px] text-slate-450 uppercase tracking-widest leading-none font-bold mt-0.5">
                Editing items for: <strong class="text-slate-700 font-mono select-all"><?php echo e($order->order_number); ?></strong>
                &mdash; <span class="text-slate-600"><?php echo e($order->name); ?></span>
            </p>
        </div>
        <div class="flex items-center gap-2 self-start sm:self-auto">
            ...
            <a href="<?php echo e(route('admin.orders.show', $order->id)); ?>" class="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all active:scale-95">
            ...
            <div class="bg-white border border-slate-200 p-4 rounded-2xl shadow-sm">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <button @click="activeCategory = '<?php echo e($category->slug); ?>'"
                            :class="activeCategory === '<?php echo e($category->slug); ?>' ? 'bg-crimson-600 text-white font-extrabold shadow' : 'text-slate-650 hover:bg-slate-100'"
                            class="w-full text-left px-3.5 py-2.5 rounded-xl text-xs flex items-center gap-2 transition-all duration-200">
        ...
        <div class="flex-grow w-full space-y-5">
            ...
            <div class="lg:hidden flex gap-1.5 overflow-x-auto pb-1 scrollbar-none select-none">
                ...
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php if($category->products->count() > 0): ?>
                    <button @click="activeCategory = '<?php echo e($category->slug); ?>'"
                        :class="activeCategory === '<?php echo e($category->slug); ?>' ? 'bg-crimson-600 text-white font-extrabold shadow' : 'bg-slate-50 text-slate-650 border border-slate-200'"
                        class="px-3.5 py-1.5 rounded-full text-[10px] uppercase tracking-wider whitespace-nowrap transition-all duration-200">
                        <?php echo e($category->name); ?>
                    ...
                    <table class="w-full border-collapse text-left text-xs">
                        ...
                        <tbody class="divide-y divide-slate-150">
                            ...
                            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php if($category->products->count() > 0): ?>
                                <!-- Category Row -->
                                <tr x-show="shouldShowCategory('<?php echo e($category->slug); ?>')"
                                    @click="toggleCategory('<?php echo e($category->slug); ?>')"
                                    ...
                                    <td colspan="6" class="py-3 px-3 sm:px-4">
                                        <div class="flex items-center justify-between text-crimson-650 tracking-wider">
                                            ...
                                            <i class="fa-solid fa-chevron-down text-[10px] text-slate-400 transition-transform duration-200"
                                               :class="collapsedCategories.has('<?php echo e($category->slug); ?>') ? '-rotate-90' : 'rotate-0'"></i>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <!-- Product Row -->
                                <tr x-show="shouldShowProduct('<?php echo e($category->slug); ?>', '<?php echo e(addslashes($product->name)); ?>')"
                                    ...
                                    <td class="py-3.5 px-3 sm:px-4 text-center">
                                        <div class="inline-flex items-center bg-slate-100 border border-slate-200 rounded-lg p-0.5 sm:p-1 select-none">
                                            <button type="button"
                                                @click="decreaseQty(<?php echo e($product->id); ?>)"
                                            ...
                                            <input type="number"
                                                @input="updateQty(<?php echo e($product->id); ?>, $event.target.value, <?php echo e($product->mrp); ?>, <?php echo e($product->selling_price); ?>)"
                                                :value="cart[<?php echo e($product->id); ?>]?.qty || 0"
                                                min="0"
                                            ...
                                            <button type="button"
                                                @click="increaseQty(<?php echo e($product->id); ?>, <?php echo e($product->mrp); ?>, <?php echo e($product->selling_price); ?>)"
                                                class="w-6 h-6 sm:w-7 sm:h-7 text-slate-650 hover:text-slate-900 hover:bg-white rounded flex items-center justify-center font-bold text-xs transition-colors shadow-sm">
                                    ...
                                    <td class="hidden md:table-cell py-3.5 px-4 text-right font-extrabold text-slate-800 pr-6">
                                        ₹<span x-text="fmt((cart[<?php echo e($product->id); ?>]?.qty || 0) * <?php echo e($product->mrp); ?>)">0.00</span>
                                    </td>
                                    ...
                                    <td class="py-3.5 px-3 sm:px-4 text-right pr-4">
                                        <span class="font-extrabold text-crimson-600"
                                            x-text="(cart[<?php echo e($product->id); ?>]?.qty || 0) > 0 ? '₹' + fmt((cart[<?php echo e($product->id); ?>]?.qty || 0) * <?php echo e($product->selling_price); ?>) : '—'">—</span>
                                    </td>
                                </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                <?php endif; ?>
...
<div x-show="totalQty > 0"
    ...
    <div class="max-w-screen-xl mx-auto flex flex-col lg:flex-row gap-3 items-center justify-between">
        ...
        <div class="flex gap-2.5 w-full lg:w-auto items-center justify-center lg:justify-end">
            ...
            <!-- Save Order -->
            <form :action="`/admin/orders/<?php echo e($order->id); ?>/edit-items`" method="POST" id="saveOrderForm">
                <?php echo csrf_field(); ?>
...
<script>
    function editOrderData() {
        return {
            ...
            initCart() {
                const existing = <?php echo json_encode($orderItems, 15, 512) ?>;
                ...
                <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $__currentLoopData = $category->products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php echo e($product->id); ?>: {
                            ...
                            category: '<?php echo e(addslashes($category->name)); ?>',
                            categorySlug: '<?php echo e($category->slug); ?>'
                        },
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            },
            ...
            submitSave() {
                ...
                form.method = 'POST';
                form.action = '/admin/orders/<?php echo e($order->id); ?>/edit-items';
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Athi\OneDrive\Desktop\crackers\resources\views/admin/orders/edit_items.blade.php ENDPATH**/ ?>