<?php $__env->startSection('title', 'Track Order Booking | ' . App\Models\Setting::get('store_name', 'Cracker Demo') . ' Sivakasi'); ?>
...
<div class="container mx-auto px-4 py-16 max-w-3xl text-slate-800">
    ...
    <div class="mt-10 space-y-8 animate-fade-in">
        ...
        <?php if($orders->count() > 0): ?>
            <h3 class="text-xs font-bold text-slate-400 uppercase tracking-widest pl-1 select-none">Found <?php echo e($orders->count()); ?> booking(s)</h3>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                ...
                <div class="bg-slate-50 border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 select-none">
                    <div>
                        ...
                        <strong class="text-sm font-extrabold text-slate-800 tracking-wider font-mono select-all"><?php echo e($order->order_number); ?></strong>
                    </div>
                    
                    <div class="flex flex-wrap gap-2">
                        <!-- Order Status Badge -->
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm <?php echo e($order->order_status_badge); ?>">
                            <i class="fa-solid fa-circle mr-1 text-[8px] opacity-80"></i>Order: <?php echo e($order->order_status); ?>

                        </span>
                        
                        <!-- Payment Status Badge -->
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider shadow-sm <?php echo e($order->payment_status_badge); ?>">
                            <i class="fa-solid fa-wallet mr-1 text-[8px] opacity-80"></i>Payment: <?php echo e($order->payment_status); ?>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-3 gap-2 text-center text-[10px] font-bold uppercase tracking-wider relative select-none">
                        ...
                        <div class="absolute top-3.5 left-[16.6%] right-[16.6%] h-1 bg-slate-100 -z-10 rounded-full border border-slate-200 overflow-hidden">
                            <div class="bg-crimson-600 h-full transition-all duration-500" 
                                 style="width: <?php echo e($order->order_status === 'pending' ? '0' : ($order->order_status === 'shipped' || $order->order_status === 'delivered' ? '100%' : '50%')); ?>%">
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            <div class="w-8 h-8 rounded-full border flex items-center justify-center mx-auto transition-all shadow-sm 
                                 <?php echo e($order->order_status !== 'cancelled' ? 'bg-crimson-50 border-crimson-500 text-crimson-600 font-extrabold' : 'bg-slate-100 border-slate-200 text-slate-400'); ?>">
                        ...
                        <div class="space-y-2">
                            <div class="w-8 h-8 rounded-full border flex items-center justify-center mx-auto transition-all shadow-sm
                                 <?php echo e($order->order_status !== 'pending' && $order->order_status !== 'cancelled' ? 'bg-crimson-50 border-crimson-500 text-crimson-600 font-extrabold' : 'bg-slate-100 border-slate-200 text-slate-400'); ?>">
                                <i class="fa-solid fa-spinner-third text-xs <?php echo e($order->order_status === 'processing' ? 'animate-spin' : ''); ?>"></i>
                            </div>
                        ...
                        <div class="space-y-2">
                            <div class="w-8 h-8 rounded-full border flex items-center justify-center mx-auto transition-all shadow-sm
                                 <?php echo e($order->order_status === 'shipped' || $order->order_status === 'delivered' ? 'bg-crimson-50 border-crimson-500 text-crimson-600 font-extrabold' : 'bg-slate-100 border-slate-200 text-slate-400'); ?>">
                    ...
                    <div class="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-3.5 text-xs font-semibold">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-150 pt-3 text-slate-500">
                            <div>
                                ...
                                <strong class="text-slate-800 text-xs"><?php echo e($order->transport_name ?? 'Not Registered Yet'); ?></strong>
                            </div>
                            <div>
                                ...
                                <?php if($order->lr_number): ?>
                                    <strong class="text-crimson-650 text-xs font-mono select-all"><?php echo e($order->lr_number); ?></strong>
                                <?php else: ?>
                    ...
                    <div class="space-y-3 text-xs">
                        ...
                        <div class="bg-slate-50 border border-slate-150 rounded-xl p-3 flex justify-between items-center text-slate-600 font-semibold">
                            <div>
                                Booking for: <strong class="text-slate-800"><?php echo e($order->name); ?></strong>
                            </div>
                            <div class="font-extrabold text-crimson-650">
                                Total net payable: ₹<?php echo e(number_format($order->net_amount, 2)); ?>
        ...
        <?php else: ?>
            ...
            <div class="bg-white border border-slate-200 p-10 rounded-2xl shadow-sm text-center space-y-3 select-none">
                ...
                <p class="text-xs text-slate-400 max-w-sm mx-auto leading-normal font-semibold">
                    We couldn't locate any order bookings matching the query: <strong class="text-slate-700 font-mono">"<?php echo e($query); ?>"</strong>. Double-check your code or try searching by phone number.
                </p>
            </div>
        <?php endif; ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Athi\OneDrive\Desktop\crackers\resources\views/track.blade.php ENDPATH**/ ?>