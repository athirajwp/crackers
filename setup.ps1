# setup.ps1 - Self-Healing Local Environment Launcher for Cracker Demo
...
if (-not (Test-Path (Join-Path $phpDir "php.exe"))) {
    ...
    # Let's try downloading from official releases
...
if (-not (Test-Path $phpIniPath)) {
    ...
    # We use php.ini-development as a template or create a custom one
    $iniContent = @"
[PHP]
engine = On
short_open_tag = Off
...
Write-Host "Creating helper batch files..." -ForegroundColor Yellow
...
Write-Host "Helper batch files created." -ForegroundColor Green
...
Write-Host "You can run the application by double-clicking 'run.bat' in the project directory." -ForegroundColor Green
