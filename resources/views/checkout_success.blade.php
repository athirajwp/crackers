        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm text-center space-y-4">
                ...
                <div class="bg-white p-3 rounded-2xl inline-block shadow-sm ring-1 ring-slate-200 select-none">
                    <img src="{{ $qrCodeUrl }}" alt="UPI QR Code" class="w-44 h-44">
                </div>
                
                <div class="text-[10px] text-slate-650 leading-normal space-y-1 select-all font-semibold">
                    <p class="font-extrabold text-slate-800">UPI Address: <span class="text-crimson-650 font-mono">{{ App\Models\Setting::get('store_upi', 'aathishacrackers@okaxis') }}</span></p>
                    <p>Payee: {{ App\Models\Setting::get('store_name', 'Aathisha Crackers') }}</p>
                    <p class="font-extrabold text-emerald-600">Payable Total: ₹{{ number_format($order->net_amount, 2) }}</p>
                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-3.5">
                ...
                <div class="text-[11px] text-slate-500 space-y-2 border-t border-slate-150 pt-3 font-semibold">
                    <div class="flex justify-between border-b border-slate-100 pb-1.5 select-all">
                        <span>Bank Name:</span>
                        <strong class="text-slate-800">{{ $bankDetails['name'] }}</strong>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-1.5 select-all">
                        <span>Account Holder:</span>
                        <strong class="text-slate-800">{{ $bankDetails['holder'] }}</strong>
                    </div>
                    <div class="flex justify-between border-b border-slate-100 pb-1.5 select-all">
                        <span>Account Number:</span>
                        <strong class="text-slate-800 font-mono">{{ $bankDetails['acc_no'] }}</strong>
                    </div>
                    <div class="flex justify-between pb-0.5 select-all">
                        <span>IFSC Code:</span>
                        <strong class="text-slate-800 font-mono">{{ $bankDetails['ifsc'] }}</strong>
                    </div>
                </div>
            </div>
